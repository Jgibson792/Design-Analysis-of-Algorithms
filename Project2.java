...
public class Project2 {
	...
	/* For team mates: 
	 * addEdge now accepts varags! addEdge(source, destinations...) 
	...
	//explore() explores all vertices in a vertex's adjacency list
...
class DirectedGraph {
    ...
    //Constructor, initialized adjacent as a hash map object, with a key pointing to a list
    DirectedGraph() {
        this.adjacent = new HashMap<Vertex, List<Vertex>>();			//vertex, adjacency list
    }

    void addVertex(String label) {
        adjacent.putIfAbsent(new Vertex(label), new ArrayList<>());
    }

    //addEdge(String source, String... destination) adds any number of destinations to the source's adjacency list
    ...
    //getAdjacent(String label) takes a label as an input and returns the vertices adjacent to the vertex with that label
    ...
    class Vertex {
        ...
        //hashCode() returns a unique hash code for the vertex
        @Override
        public int hashCode() {
            int result = 1;
        ...
        //equals(Object obj) takes an object to see if it is equal to this vertex
        @Override
        ...
        //toString() returns the label of the vertex when called
        @Override
